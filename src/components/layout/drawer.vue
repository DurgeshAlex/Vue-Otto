    ...
    <md-app>
      ...
      <md-app-drawer :md-active.sync="menuVisible" md-persistent="mini" class="md-primary">
        ...
        <md-list-item v-if="menuVisible" class="md-list-label">
          ...
          <md-list-item v-for="item in drawerItems.market.links" :key="item.label" :class="(item.child ? 'md-expand' : '') + (item.label == setting.selectedSidebar ? ' active' : '')" @click="$setActiveSidebar({
            breadcrumb: item.label,
            selectedSidebar: item.label
          })" :md-expand="item.child ? true : false">
            <router-link v-if="!item.child" :to="item.path" class="flex link-area">
            ...
            <md-list slot="md-expand" v-if="item.child">
              <router-link :to="sub.path" class="flex link-area" v-for="sub in item.child" :key="sub.label">
                <md-list-item class="md-inset" >{{sub.label}}</md-list-item>
              </router-link>
            </md-list>
          </md-list-item>
          <md-list-item v-if="menuVisible" class="md-list-label">
          ...
          <md-list-item v-for="item in drawerItems.ottu_rent.links" :key="item.label" :class="(item.child ? 'md-expand' : 'no-expand') + (item.label == setting.selectedSidebar ? ' active' : '')" @click="$setActiveSidebar({
            breadcrumb: item.label,
            selectedSidebar: item.label
          })" :md-expand="item.child ? true : false">
            <router-link v-if="!item.child" :to="item.path" class="flex link-area">
         ...
         <md-list-item v-for="item in drawerItems.settings.links" :key="item.label" :class="(item.child ? 'md-expand' : 'no-expand') + (item.label == setting.selectedSidebar ? ' active' : '')" @click="item.child ? $setActiveSidebar({
            breadcrumb: item.label,
            selectedSidebar: item.label
          }) : ''" :md-expand="!!item.child">
            <router-link v-if="!item.child" :to="item.path" class="flex link-area">
              <md-icon>{{item.icon}}</md-icon>
              <span class="md-list-item-text">{{item.label}}</span>
              <md-icon class="dropdown-icon" v-if="item.child">keyboard_arrow_down</md-icon>
